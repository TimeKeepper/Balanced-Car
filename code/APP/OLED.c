...
{
    RCC_APB2PeriphClockCmd(GET_GPIO_RCC(OLED_SCL_PORT)|GET_GPIO_RCC(OLED_SDA_PORT), ENABLE);
...
void OLED_ShowChi(uint8_t Line, uint8_t Column, char Char)
{      	
	uint8_t i;
	OLED_SetCursor((Line - 1) * 2, (Column - 1) * 8);		//设置光标位置在上半部分
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(OLED_chi[Char - ' '][i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部分
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(OLED_chi[Char - ' '][i + 8]);		//显示下半部分内容
	}
}
