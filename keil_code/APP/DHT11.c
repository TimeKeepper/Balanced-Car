...
void DHT11_IO_OUT (void){ //温湿度模块输出函数
    ...
    GPIO_InitStructure.GPIO_Pin = DHT11_Pin; //选择端口号（0~15或all）                        
...
void DHT11_RST (void){ 						//DHT11端口复位，发出起始信号（IO发送）
	DHT11_IO_OUT();							//端口为输出
	GPIO_ResetBits(DHT11_PORT,DHT11_Pin); 	//使总线为低电平
	delay_ms(20); 							//拉低至少18ms						
	GPIO_SetBits(DHT11_PORT,DHT11_Pin); 		//使总线为高电平							
	delay_us(30); 							//主机拉高20~40us
}

void DHT11_WaitForReAct(u8 flag,u8* retry){
	while ((GPIO_ReadInputDataBit(DHT11_PORT,DHT11_Pin) == flag) && *retry<100)	//DHT11会拉低40~80us
	{
		(*retry)++;
        delay_us(1);
    }
...
u8 DHT11_Read_Bit(void)
{
    ...
    DHT11_WaitForReAct(0,&retry);
    delay_us(40);//等待40us
    if(GPIO_ReadInputDataBit(DHT11_PORT,DHT11_Pin) == 1)       //用于判断高低电平，即数据1或0
        return 1;
    else
        return 0;
}
